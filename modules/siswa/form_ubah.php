...
if (isset($_GET['id'])) {
    ...
    // sql statement untuk menampilkan data dari tabel "tbl_siswa" berdasarkan "id_siswa" dan tabel "tbl_asrama" berdasarkan "id_siswa"
    $query = mysqli_query($mysqli, "SELECT a.id_siswa, a.tanggal_daftar, a.asrama, a.nama_lengkap, a.jenis_kelamin, a.alamat, a.email, a.whatsapp, a.foto_profil, b.nama_asrama 
                                    FROM tbl_siswa as a INNER JOIN tbl_asrama as b ON a.asrama=b.id_asrama 
        ...
        <div class="row">
            ...
            <div class="col-xl-6">
                <div class="mb-3 ps-xl-3">
                    ...
                    <select name="asrama" class="form-select" autocomplete="off" required>
                        <option value="<?php echo $data['asrama']; ?>"><?php echo $data['nama_asrama']; ?></option>
                        <option disabled value="">-- Pilih --</option>
                        <?php
                        // sql statement untuk menampilkan data dari tabel "tbl_asrama"
                        $query_asrama = mysqli_query($mysqli, "SELECT id_asrama, nama_asrama FROM tbl_asrama ORDER BY nama_asrama ASC")
                                                              or die('Ada kesalahan pada query tampil data : ' . mysqli_error($mysqli));
                        // ambil data hasil query
                        while ($data_asrama = mysqli_fetch_assoc($query_asrama)) {
                            // tampilkan data
                            echo "<option value='$data_asrama[id_asrama]'>$data_asrama[nama_asrama]</option>";
                        }
                        ?>
                    </select>
                    <div class="invalid-feedback">Asrama tidak boleh kosong.</div>
                </div>
            </div>
        </div>
