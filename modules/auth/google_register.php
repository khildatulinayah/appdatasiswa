...
// File ini menghandle register via Google OAuth2
...
try {
    ...
    // Add register parameter to distinguish from login
    $auth_url .= '&state=register_' . bin2hex(random_bytes(8));
    
    // Store session state for security
    $_SESSION['google_oauth_state'] = 'register';
    ...
    header('Location: ' . $base_url . '/register.php?error=google_auth_failed');
    exit();
}
