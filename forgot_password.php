...
// Redirect to simple version (no database dependency issues)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!validate_secure_csrf_token($_POST['csrf_token'] ?? '')) {
        header('Location: forgot_password.php?error=csrf_invalid');
        exit();
    }
    
    // Rate limiting check
    if (!check_rate_limit('forgot_password', 3, 300)) { // 3 requests per 5 minutes
        header('Location: forgot_password.php?error=rate_limit_exceeded');
        exit();
    }
    ...
    if ($result['success']) {
        ...
        if ($email_sent) {
            ...
            header('Location: forgot_password.php?success=code_sent');
            exit();
        } else {
...
if (isset($_POST['verify_code'])) {
    ...
    if (empty($token) || empty($verification_code)) {
        header('Location: forgot_password.php?error=missing_code');
        exit();
    }
    ...
    if ($result['success']) {
        ...
        header('Location: forgot_password.php?error=invalid_code');
        exit();
    }
}
...
if (isset($_POST['reset_password'])) {
    if (!($_SESSION['reset_verified'] ?? false)) {
        header('Location: forgot_password.php?error=not_verified');
        exit();
    }
    ...
    if (empty($new_password) || empty($confirm_password)) {
        header('Location: forgot_password.php?error=empty_password');
        exit();
    }
    ...
    if ($result['success']) {
        ...
        header('Location: forgot_password.php?error=reset_failed');
        exit();
    }
}
?>
                            ...
                            <form method="post">
                                ...
                                <div class="text-center mt-3">
                                    <small class="text-muted">
                                        Tidak menerima kode? 
                                        <a href="forgot_password.php?reset=1" class="text-decoration-none">Kirim ulang</a>
                                    </small>
                                </div>
                            </form>
                            <?php endif; ?>
